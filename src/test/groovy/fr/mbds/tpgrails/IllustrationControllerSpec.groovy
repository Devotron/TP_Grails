...
@TestFor(IllustrationController)
@Mock(Illustration)
class IllustrationControllerSpec extends Specification {
    ...
    void "Test the save action correctly persists an instance"() {
        ...
        when:"The save action is executed with a valid instance"
            ...
            illustration = new Illustration(params)
    ...
    void "Test the update action performs an update on a valid domain instance"() {
        ...
        when:"A valid domain instance is passed to the update action"
            response.reset()
            populateValidParams(params)
            illustration = new Illustration(params).save(flush: true)
            controller.update(illustration)

        then:"A redirect is issued to the show action"
            illustration != null
            response.redirectedUrl == "/illustration/show/$illustration.id"
            flash.message != null
    }
    ...
    void "Test that the delete action deletes an instance if it exists"() {
        ...
        when:"A domain instance is created"
            response.reset()
            populateValidParams(params)
            def illustration = new Illustration(params).save(flush: true)

        then:"It exists"
            Illustration.count() == 1

        when:"The domain instance is passed to the delete action"
            controller.delete(illustration)

        then:"The instance is deleted"
            Illustration.count() == 0
            response.redirectedUrl == '/illustration/index'
            flash.message != null
    }
}
